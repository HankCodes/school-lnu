...
module.exports.errorLogger = async (message, stackTrace) => {
    try {
        console.log(path.resolve(__dirname + '/../log/error.txt'))
        if (process.env.NODE_ENV === 'production') {
            await appendFile(path.resolve(__dirname + '/../log/error.txt'), formatErrorMessage(message, stackTrace))
        } else {
...
module.exports.accessLogger = async (message) => {
    try {
        if (process.env.NODE_ENV === 'production') {
            await appendFile(path.resolve(__dirname + '/../log/access.txt'), formatAccessLogMessage(message))
        } else {
